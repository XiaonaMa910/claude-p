...
- **`--model` defaults to your account's default model** (upstream hard-coded
  `sonnet`), matching native `claude -p` behavior when the flag is omitted.

Install this fork directly:

```bash
python3 -m pip install "git+https://github.com/XiaonaMa910/claude-p.git"
```
...
```bash
uv tool install "git+https://github.com/XiaonaMa910/claude-p.git" --force
...
```bash
python -m pip install "git+https://github.com/XiaonaMa910/claude-p.git"
claude-p "Respond exactly: CLAUDE_P_OK" --tools '' --timeout-sec 45 --quiet-after-sec 2
```
...
- Global console command after `uv tool install "git+https://github.com/XiaonaMa910/claude-p.git"` or `python -m pip install "git+https://github.com/XiaonaMa910/claude-p.git"`.
...
| Symptom | Cause | Fix |
|---|---|---|
| `zsh: command not found: pip` | Your shell has no `pip` command on `PATH`. | Use `python -m pip ...` or `uv tool install "git+https://github.com/XiaonaMa910/claude-p.git"`. |
| `No matching distribution found for claude-p` | Usually an old Python/pip pair, or Python < 3.10. | Check `python3 --version`; install with `uv tool install "git+https://github.com/XiaonaMa910/claude-p.git"`, or use a modern Python such as 3.11/3.12. |
...
| Import works in one terminal but not another | `pip` and `python` point to different environments. | Always run `python -m pip install "git+https://github.com/XiaonaMa910/claude-p.git"` inside the environment you will use. |
...
may not see modern wheels correctly. Prefer `uv tool install "git+https://github.com/XiaonaMa910/claude-p.git"`, Homebrew
...
- 自动注入 statusline 配置以捕获 cost/限流 payload（你自己传了 `--settings` 时
  不注入），临时快照文件运行结束即删除。
- **`--model` 缺省时使用账号默认模型**（上游硬编码为 `sonnet`），与原生
  `claude -p` 不传该参数时的行为一致。
...
- 通过 `uv tool install "git+https://github.com/XiaonaMa910/claude-p.git"` 或 `python -m pip install "git+https://github.com/XiaonaMa910/claude-p.git"` 安装后提供
  全局命令 `claude-p`。
...
| 现象 | 原因 | 解决方式 |
|---|---|---|
| `zsh: command not found: pip` | 当前 shell 里没有 `pip` 命令。 | 用 `python -m pip ...`，或直接 `uv tool install "git+https://github.com/XiaonaMa910/claude-p.git"`。 |
| `No matching distribution found for claude-p` | 通常是 Python/pip 太旧，或 Python 版本低于 3.10。 | 先看 `python3 --version`；推荐 `uv tool install "git+https://github.com/XiaonaMa910/claude-p.git"`，或换 Python 3.11/3.12。 |
| 安装后 `claude-p: command not found` | Python scripts 目录不在 `PATH`。 | 把 `~/.local/bin` 加到 `PATH`，或用 `uv tool run claude-p ...`。 |
| 一个终端能 import，另一个不行 | `pip` 和 `python` 指向不同环境。 | 在目标环境里始终使用 `python -m pip install "git+https://github.com/XiaonaMa910/claude-p.git"`。 |
...
`uv tool install "git+https://github.com/XiaonaMa910/claude-p.git"`、Homebrew Python，或 conda/venv。
